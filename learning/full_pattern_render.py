...
Extends pad_pattern_render by also drawing existing traces as 2-pixel lines.
Colour scheme encodes both net relationship and layer:

  - Bright red  (255,  50,  50): foreign net F.Cu trace
  - Orange      (255, 150,   0): foreign net B.Cu trace
  - Bright green ( 50, 255,  50): GND F.Cu trace
  - Cyan         (  0, 200, 200): GND B.Cu trace
  - Blue dot:    same net pad
  - Red dot:     foreign net pad
  - Green dot:   GND pad
  - Black:       empty space
...
# Layer-aware trace colours
COL_FOREIGN_FCU = (255,  50,  50)   # foreign net F.Cu — bright red
COL_FOREIGN_BCU = (255, 150,   0)   # foreign net B.Cu — orange
COL_GND_FCU     = ( 50, 255,  50)   # GND F.Cu — bright green
COL_GND_BCU     = (  0, 200, 200)   # GND B.Cu — cyan
...
def count_visible_obstacle_traces(tracks, route_net, source_pt, dest_pt,
                                   margin_ratio=MARGIN_RATIO):
    """Count non-route-net traces visible in the rendering window.

    Used as a collection quality filter — routes with crossing traces are
    valuable patterns even if they have few obstacle pads.
    """
    sx, sy = source_pt
    dx, dy = dest_pt
    ...
    for t in tracks:
        net = t.get("net", "")
        if not net or net == route_net:
            continue

        rx1, ry1 = _rotate(t["x1"], t["y1"])
        rx2, ry2 = _rotate(t["x2"], t["y2"])

        if (min_x <= rx1 <= max_x and min_y <= ry1 <= max_y) or \
           (min_x <= rx2 <= max_x and min_y <= ry2 <= max_y):
            count += 1
...
def _draw_traces(draw, tracks, route_net, min_x, min_y, width, height):
    """Draw all tracks within the window as 2-pixel lines, layer-aware colour."""
    for t in tracks:
        net = t.get("net", "")
        if not net or net == route_net:
            continue
        ...
        is_bcu = ("B." in layer) if isinstance(layer, str) else (layer == 1)

        if _is_gnd_net(net):
            colour = COL_GND_BCU if is_bcu else COL_GND_FCU
        else:
            colour = COL_FOREIGN_BCU if is_bcu else COL_FOREIGN_FCU
        ...
        draw.line([(px1, py1), (px2, py2)], fill=colour, width=2)
...
def render_full_pattern(pads, tracks, route_net, source_pt, dest_pt,
    ...
    along +X. Traces are coloured by net relationship and layer.
    ...
    # Draw traces first — pads drawn on top
